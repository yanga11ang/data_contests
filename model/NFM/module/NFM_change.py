...
class NFM(BaseModule):
    ...
    def __init__(self,field_size, feature_sizes, embedding_size = 4, is_shallow_dropout = True, dropout_shallow = [0.5],
                 ...
                 drop_shallow =False,addition_feature_size = 0,
                 deep_layers_activation = 'relu', n_epochs = 64, batch_size = 256, learning_rate = 0.003,
                 optimizer_type = 'adam', is_batch_norm = False, random_seed = 970801, weight_decay = 0.0,
        ...
        # my addition
        self.drop_shallow = drop_shallow
        self.addition_feature_size = addition_feature_size
        ...
        if self.interation_type:
            self.linear_1 = nn.Linear(self.embedding_size + self.addition_feature_size, deep_layers[0])
        else:
    ...
    def forward(self, data):
        ...
        addition_feature = data['addition_feature']
        """ fm part """
        if self.use_fm:
            # .t() 转置
            if not self.drop_shallow:
            ...
            if not self.drop_shallow and self.is_shallow_dropout:
        ...
        if self.use_fm and self.interation_type:
            if self.addition_feature_size==0:
                deep_emb = fm_second_order
            else:
                deep_emb = torch.cat([fm_second_order,addition_feature], 1)
        elif self.use_ffm and self.interation_type:
            if self.addition_feature_size == 0:
                deep_emb = ffm_second_order
            else:
                deep_emb = torch.cat([ffm_second_order,addition_feature], 1)
        elif self.use_fm:
        ...
        """
            sum
        """
        if self.use_fm and not self.drop_shallow :
            total_sum = self.bias+ torch.sum(fm_first_order,1) + torch.sum(x_deep,1)
        elif self.use_fm and self.drop_shallow:
            total_sum = self.bias  + torch.sum(x_deep, 1)
        elif self.use_ffm and not self.drop_shallow :
    ...
    def fit(self, Xi_train, Xv_train,  y_train,addition_feature=None,eval_set=None,
            early_stopping_rounds=None, verbose=None,eval_metric=['f1']):

        trainDataLoader = DataLoader(Xi_train,Xv_train,y_train,addition_feature=addition_feature,batch_size= self.batch_size)
        ...
        if eval_set is not None:
            ...
            for valid_data in eval_set:
                ...
                if len(valid_data) >= 4:
                    addition_feature_valid = valid_data[3]
                else:
                    addition_feature_valid=None
                validDataLoader_list.append(DataLoader(Xi_valid, Xv_valid, y_valid,addition_feature=addition_feature_valid, batch_size=self.batch_size))
    ...
    def predict(self, Xi=None, Xv=None ,addition_feature=None,data=None, num_iteration=-1):
        return (self.predict_proba(Xi=Xi, Xv=Xv,addition_feature=addition_feature, data=data,num_iteration=num_iteration) > 0.5)

    def predict_proba(self, Xi=None, Xv=None,addition_feature=None, data=None,num_iteration=-1):
        if num_iteration!=-1:
        ...
        if data is None:
            ...
            data = {"Xi": Xi, "Xv": Xv}
            if addition_feature is None:
                data['addition_feature'] =None
            else:
                addition_feature = np.array(addition_feature)
                data['addition_feature'] = Variable(torch.FloatTensor(addition_feature))

        if self.use_cuda and torch.cuda.is_available():
            data['Xi'], data['Xv'] = data['Xi'].cuda(), data['Xv'].cuda()
            data['addition_feature'] = data['addition_feature'].cuda()
        model = self.eval()
        with torch.no_grad():
